...
# creates a connection to the database
    ...
    db = mysql.connector.connect(user=connection_detail.user,
    ...
    # mycursor.execute("DROP TABLE TomerSteinmetz_test_Female")
    
    # Requirement_1
    # helper method, put the json fields as the database fields
    init_table(response, mycursor)
    # helper method, by using randomuser api, generates 4500 users and inSert them to the dataset at once
    insert_users(4500,mycursor,db)
    # Requirement_2
    # splitting the data to 2 gender datasets by creating two tables 
    mycursor.execute("CREATE TABLE TomerSteinmetz_test_Female LIKE Users")
    mycursor.execute("INSERT INTO TomerSteinmetz_test_Female SELECT *  FROM Users WHERE gender= 'female';")
    db.commit()
    mycursor.execute("CREATE TABLE TomerSteinmetz_test_male LIKE Users")  
    mycursor.execute("INSERT INTO TomerSteinmetz_test_male SELECT * FROM Users WHERE gender= 'male';")
    db.commit()
    ...
    for age in range(10,110,10):
        table_name = "TomerSteinmetz_test_" + str(age // 10)
        query = f"{age} AND `dob.age` < {age + 10};"
        mycursor.execute(f"CREATE TABLE " + table_name +" LIKE Users")
        mycursor.execute(f"INSERT INTO " + table_name + " SELECT * FROM Users WHERE `dob.age` >= " + query) 

    db.commit()
    # Requirement_5
    # creat from the 20 last registered females the dataset
    # insert the last 20 registered males to the dataset 
    mycursor.execute("CREATE TABLE TomerSteinmetz_test_20 LIKE Users")
    mycursor.execute("INSERT INTO TomerSteinmetz_test_20 SELECT * FROM TomerSteinmetz_test_Female ORDER BY (`registered.date`) DESC LIMIT 20")
